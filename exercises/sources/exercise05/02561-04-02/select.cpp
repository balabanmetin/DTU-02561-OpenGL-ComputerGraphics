...
void SelectBuffer::buildFrameBufferObject(){
	...
	glGenRenderbuffers(1, &renderBufferId); //we don't need to attach a renderbuffer. we don't use depth info. 
	...
	glBindFramebuffer(GL_DRAW_FRAMEBUFFER, 0);//draw screen. read from fbo.
}
...
void SelectBuffer::release(){
	glBindFramebuffer(GL_FRAMEBUFFER, 0);//unbind. switch main framebuffer
	glFlush();
}
